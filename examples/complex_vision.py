...
def plot_complex_with_stats(ds_complex, mesh):
    ...
    fig.suptitle(
        "Option 1: Complex Reconstruction with Statistics",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
...
def main():
    ...
    print(
        f"    Real part:      mean={np.mean(np.real(ds)):.6f}, max={np.max(np.real(ds)):.6f}"
    )
    print(
        f"    Imag part:      mean={np.mean(np.imag(ds)):.6f}, max={np.max(np.imag(ds)):.6f}"
    )
    print(
        f"    Magnitude:      mean={np.mean(np.abs(ds)):.6f}, max={np.max(np.abs(ds)):.6f}"
    )
